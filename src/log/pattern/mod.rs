...
use crate::log::{Style, Color, Encode};

use self::parser::{Parameters, Alignment, Piece, Parser};

use super::{NEWLINE, ProxyRecord};
...
impl<'a> crate::log::Write for MaxWidthWriter<'a> {
...
impl<W: crate::log::Write> LeftAlignWriter<W> {
...
impl<W: crate::log::Write> io::Write for LeftAlignWriter<W> {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        match self.w.write(buf) {
            Ok(len) => {
...
impl<W: crate::log::Write> crate::log::Write for LeftAlignWriter<W> {
    fn set_style(&mut self, style: &Style) -> io::Result<()> {
        self.w.set_style(style)
    }
}
...
impl<W: crate::log::Write> RightAlignWriter<W> {
    fn finish(mut self) -> io::Result<()> {
        for _ in 0..self.to_fill {
            write!(self.w, "{}", self.fill)?;
        }
...
impl<W: crate::log::Write> io::Write for RightAlignWriter<W> {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
...
impl<W: crate::log::Write> crate::log::Write for RightAlignWriter<W> {
    fn set_style(&mut self, style: &Style) -> io::Result<()> {
...
impl Chunk {
    fn encode(&self, w: &mut dyn crate::log::Write, record: &ProxyRecord) -> io::Result<()> {
        match *self {
...
impl Encode for PatternEncoder {
    fn encode(&self, w: &mut dyn crate::log::Write, record: &ProxyRecord<'_>) -> io::Result<()> {
...
mod tests {
    ...
    use crate::log::writer::simple::SimpleWriter;
    use crate::log::Encode;
    ...
    fn left_align() {
        ...
        pw.encode(
            &mut SimpleWriter(&mut buf),
            &Record::builder().args(format_args!("foo")).build().into(),
        )
        .unwrap();
        assert_eq!(buf, b"foo~~");
        ...
        pw.encode(
            &mut SimpleWriter(&mut buf),
            &Record::builder().args(format_args!("foobar!")).build().into(),
        )
        .unwrap();
        assert_eq!(buf, b"foobar");
    }
    ...
    fn quote_braces_with_backslash() {
        ...
        pw.encode(
            &mut SimpleWriter(&mut buf),
            &Record::builder().level(Level::Info).build().into(),
        )
        .unwrap();
